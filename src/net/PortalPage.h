...
<script>
const pt=document.cookie.split(';').some(x=>x.trim()==='m5epub_lang=pt');document.documentElement.lang=pt?'pt-BR':'en';const t=pt?{title:'Biblioteca do M5Paper',folder:'Nova pasta',upload:'Enviar EPUB',drop:'Arraste arquivos EPUB aqui',free:'livres',empty:'Pasta vazia',del:'Apagar',confirm:'Apagar este item?',name:'Nome da pasta:',failed:'Falha'}:{title:'M5Paper Library',folder:'New folder',upload:'Upload EPUB',drop:'Drop EPUB files here',free:'free',empty:'Empty folder',del:'Delete',confirm:'Delete this item?',name:'Folder name:',failed:'Failed'};document.title=t.title;
